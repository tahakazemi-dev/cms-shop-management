/**
 * Carpet Manager System - Final Fixed Version
 */
...
function printInvoice(transId) {
    ...
    // HTML فاکتور با استایل داخلی برای اطمینان از عدم تداخل
    printArea.innerHTML = `
        <style>
            @media print {
                @page { margin: 0; }
            ...
            .invoice-container {
                width: 72mm; /* عرض ایمن برای پرینتر */
                margin: 0 auto;
                padding: 5px 2px;
                font-family: 'Tahoma', sans-serif;
                font-size: 10px; /* فونت ریز */
                color: black;
                direction: rtl;
                line-height: 1.4;
            }
        ...
        <div class="invoice-container">
            ...
            <div class="inv-total-row">
                جمع کل: ${transaction.totalPrice.toLocaleString()}
            </div>
...
// محاسبه قیمت لحظه‌ای در فرم (فقط نمایشی)
function calculateLivePrice() {
    ...
    if (pid && amt) {
        ...
        if (p) {
            ...
            t += (t * (per / 100));
            document.getElementById('liveFinalPrice').innerText = Math.round(t).toLocaleString() + ' ریال';
        }
    } else {
        document.getElementById('liveFinalPrice').innerText = '0 ریال';
    }
...
function addProduct() {
    ...
    // گرفتن درصد (این خط رو که قبلا اضافه کردیم)
    ...
    // 4. خالی کردن فیلدها (اینجا باید اضافش کنی)
...
function renderTransactionsTable(list) {
    ...
    list.forEach(t => {
        tb.innerHTML += `<tr>
            ...
            <td>${t.date}</td>
            <td class="actions-cell">
                <button class="btn-print" onclick="printInvoice(${t.id})">🖨️</button>
                <button class="btn-danger" onclick="delTrans(${t.id})">🗑️</button>
            </td>
        </tr>`;
    });
}
...
function populateCustomSelect() { const list = document.getElementById('dropdownList'); list.innerHTML = '';[...products].sort((a, b) => a.name.localeCompare(b.name)).forEach(p => { const d = document.createElement('div'); d.className = 'option-item'; d.innerHTML = `<strong>${p.name}</strong> <small>(${p.stock})</small>`; d.dataset.search = (p.name + ' ' + p.brand).toLowerCase(); d.onclick = () => selectProduct(p); list.appendChild(d); }); }
function selectProduct(p) { document.getElementById('productSearchInput').value = p.name; document.getElementById('selectedProductId').value = p.id; document.getElementById('dropdownList').classList.remove('show'); document.getElementById('unitPriceLabel').innerText = `قیمت: ${p.sellPrice.toLocaleString()}`; document.getElementById('stockLabel').innerText = `موجودی: ${p.stock}`; document.getElementById('saleMeters').focus(); calculateLivePrice(); }
...
function renderTrash() {
    ...
    // بخش فاکتورها (اینجا دکمه حذف کم بود که اضافه شد)
...
// 1. محاسبه‌گر اختصاصی برای بخش ویرایش (حل مشکل کار نکردن درصد)
...
// 2. باز کردن مودال (حل مشکل باز نشدن)
window.openEditModal = function (id) {
    ...
    // اینجا خیلی مهم است: اگر قبلا درصدی نداشت، صفر بگذار تا ارور ندهد
...
window.saveEditProduct = function () {
    ...
    if (p) {
        ...
        calculateInventoryStats();
        updateAccounting();
        populateCustomSelect(); // آپدیت لیست فروش
...
// --- توابع حذف از سطل زباله (دکمه‌های قرمز) ---
...
// --- توابع سبد خرید (اگر دکمه ضربدر سبد خرید کار نمی‌کند) ---

// حذف کالا از سبد خرید
window.removeFromCart = function (index) {
    cart.splice(index, 1); // حذف آیتم از آرایه سبد
    renderCart(); // نمایش مجدد سبد
};

// پاک کردن کل سبد
window.clearCart = function () {
    if (confirm('آیا کل سبد خرید پاک شود؟')) {
        cart = [];
        renderCart();
    }
};
...
// محاسبه خودکار قیمت بر اساس درصد
function autoCalcSell(prefix) {
    // گرفتن قیمت خرید (حذف کاما)
